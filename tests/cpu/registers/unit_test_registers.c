 ...
 * @brief File containing tests to verify the functionality of the registers
 * module
 */
#include <stdbool.h>
#include <stddef.h>
...
static bool validate_memory_isolation(const byte *p_reg_before,
...
static bool test_core_registers(void);
static bool test_core_register_AF(void);
static bool test_core_register_BC(void);
static bool test_core_register_DE(void);
static bool test_core_register_HL(void);
static bool test_core_register_SP(void);
static bool test_core_register_PC(void);
static bool test_core_register(word (*reg_get)(void),
                               void (*reg_set)(const word value),
                               const size_t reg_offset, const word bitmask);

static bool test_virtual_registers(void);
static bool test_virtual_register_A(void);
static bool test_virtual_register_F(void);
static bool test_virtual_register_B(void);
static bool test_virtual_register_C(void);
static bool test_virtual_register_D(void);
static bool test_virtual_register_E(void);
static bool test_virtual_register_H(void);
static bool test_virtual_register_L(void);
static bool test_virtual_register(byte (*reg_get)(void),
                                  void (*reg_set)(const byte value),
                                  const size_t reg_offset, const byte bitmask);
...
static bool test_register_composition(void (*core_reg_set)(const word value),
...
static bool test_flags(void);
static bool test_flag_z(void);
static bool test_flag_n(void);
static bool test_flag_h(void);
static bool test_flag_c(void);
static bool test_flag(bool (*flag_get)(), void (*flag_set)(),
                      void (*flag_clear)(), const int bit_index);

int main(void) {
  // 0 indicates a success, 1 a failure
  return (test_registers_module() == true) ? 0 : 1;
}
  ...
  for (size_t i = 0; i < sizeof(struct registers); i++) {
    if (i < reg_offset || i >= (reg_offset + block_size)) {
      ASSERT_EQ(p_reg_before[i], p_reg_after[i],
                "Isolation Failure at byte %zu. Should have been 0x%.2X but "
                "was 0x%.2X",
...
static bool test_core_register_AF(void) {
  return test_core_register(registers_get_AF, registers_set_AF,
                            offsetof(struct registers, AF), 0xFFF0);
}

static bool test_core_register_BC(void) {
  return test_core_register(registers_get_BC, registers_set_BC,
                            offsetof(struct registers, BC), 0xFFFF);
}

static bool test_core_register_DE(void) {
  return test_core_register(registers_get_DE, registers_set_DE,
                            offsetof(struct registers, DE), 0xFFFF);
}

static bool test_core_register_HL(void) {
  return test_core_register(registers_get_HL, registers_set_HL,
                            offsetof(struct registers, HL), 0xFFFF);
}

static bool test_core_register_SP(void) {
  return test_core_register(registers_get_SP, registers_set_SP,
                            offsetof(struct registers, SP), 0xFFFF);
}

static bool test_core_register_PC(void) {
  return test_core_register(registers_get_PC, registers_set_PC,
                            offsetof(struct registers, PC), 0xFFFF);
}
...
static bool test_core_register(word (*reg_get)(void),
  ...
  for (dword test_value = 0x0000; test_value <= 0xFFFF; test_value++) {
    ...
    ASSERT_EQ(set_value, expected_value,
              "Physical Memory Mismatch at offset %zu. Should have been 0x%.4X "
              "but was 0x%.4X.",
    ...
    ASSERT_TRUE(validate_memory_isolation(p_reg_before, p_reg_after, reg_offset,
                                          sizeof(word)),
...
static bool test_virtual_register_A(void) {
  return test_virtual_register(registers_get_A, registers_set_A,
                               offsetof(struct registers, A), 0xFF);
}

static bool test_virtual_register_F(void) {
  return test_virtual_register(registers_get_F, registers_set_F,
                               offsetof(struct registers, F), 0xF0);
}

static bool test_virtual_register_B(void) {
  return test_virtual_register(registers_get_B, registers_set_B,
                               offsetof(struct registers, B), 0xFF);
}

static bool test_virtual_register_C(void) {
  return test_virtual_register(registers_get_C, registers_set_C,
                               offsetof(struct registers, C), 0xFF);
}

static bool test_virtual_register_D(void) {
  return test_virtual_register(registers_get_D, registers_set_D,
                               offsetof(struct registers, D), 0xFF);
}

static bool test_virtual_register_E(void) {
  return test_virtual_register(registers_get_E, registers_set_E,
                               offsetof(struct registers, E), 0xFF);
}

static bool test_virtual_register_H(void) {
  return test_virtual_register(registers_get_H, registers_set_H,
                               offsetof(struct registers, H), 0xFF);
}

static bool test_virtual_register_L(void) {
  return test_virtual_register(registers_get_L, registers_set_L,
                               offsetof(struct registers, L), 0xFF);
}
...
static bool test_virtual_register(byte (*reg_get)(void),
  ...
  for (word test_value = 0x00; test_value <= 0xFF; test_value++) {
    ...
    ASSERT_EQ(set_value, expected_value,
              "Physical Memory Mismatch at offset %zu. Should have been 0x%.2X "
              "but was 0x%.2X",
              reg_offset, expected_value, set_value);
...
static bool test_composition_AF(void) {
  return test_register_composition(registers_set_AF, registers_get_AF,
                                   registers_set_A, registers_get_A,
                                   registers_set_F, registers_get_F, 0xFFF0);
}

static bool test_composition_BC(void) {
  return test_register_composition(registers_set_BC, registers_get_BC,
                                   registers_set_B, registers_get_B,
                                   registers_set_C, registers_get_C, 0xFFFF);
}

static bool test_composition_DE(void) {
  return test_register_composition(registers_set_DE, registers_get_DE,
                                   registers_set_D, registers_get_D,
                                   registers_set_E, registers_get_E, 0xFFFF);
}

static bool test_composition_HL(void) {
  return test_register_composition(registers_set_HL, registers_get_HL,
                                   registers_set_H, registers_get_H,
                                   registers_set_L, registers_get_L, 0xFFFF);
}
...
static bool test_register_composition(void (*core_reg_set)(const word value),
  ...
  for (dword test_value = 0x0000; test_value <= 0xFFFF; test_value++) {
    ...
    ASSERT_EQ(returned_value_hi, expected_value_hi,
              "Composition Failure. High byte mismatch. Expected 0x%.2X but "
              "was 0x%.2X",
              expected_value_hi, returned_value_hi);
    ASSERT_EQ(returned_value_lo, expected_value_lo,
              "Composition Failure. Low byte mismatch. Expected 0x%.2X but was "
              "0x%.2X",
...
static bool test_flag_z(void) {
  return test_flag(registers_get_flag_z, registers_set_flag_z,
                   registers_clear_flag_z, 7);
}

static bool test_flag_n(void) {
  return test_flag(registers_get_flag_n, registers_set_flag_n,
                   registers_clear_flag_n, 6);
}

static bool test_flag_h(void) {
  return test_flag(registers_get_flag_h, registers_set_flag_h,
                   registers_clear_flag_h, 5);
}

static bool test_flag_c(void) {
  return test_flag(registers_get_flag_c, registers_set_flag_c,
                   registers_clear_flag_c, 4);
}
...
static bool test_flag(bool (*flag_get)(), void (*flag_set)(),
  ...
  // Validate Getter
  ASSERT_TRUE(flag_get(),
              "Getter Mismatch. Should have been set but was not set");
  ...
  // Validate Getter
  ASSERT_FALSE(flag_get(),
               "Getter Mismatch. Should have been not set but was set");
