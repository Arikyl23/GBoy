...
static inline void cpu_generic_OR_r8_u8(byte* dst, const byte src) {
    struct alu_result result = alu_or_8b(*dst, src);
    *dst                     = result.result.byte;
    registers_set_flags(&m_reg, result.flags.z, false, false, false);
}
...
static void cpu_opcode_DI(void) { m_reg.IME = false; }
static void cpu_opcode_EI(void) { m_reg.IME = true; }
// Misc
...
static void cpu_opcode_OR_rA_rB(void) { cpu_generic_OR_r8_u8(&m_reg.A, m_reg.B); }
static void cpu_opcode_OR_rA_rC(void) { cpu_generic_OR_r8_u8(&m_reg.A, m_reg.C); }
static void cpu_opcode_OR_rA_rD(void) { cpu_generic_OR_r8_u8(&m_reg.A, m_reg.D); }
static void cpu_opcode_OR_rA_rE(void) { cpu_generic_OR_r8_u8(&m_reg.A, m_reg.E); }
static void cpu_opcode_OR_rA_rH(void) { cpu_generic_OR_r8_u8(&m_reg.A, m_reg.H); }
static void cpu_opcode_OR_rA_rL(void) { cpu_generic_OR_r8_u8(&m_reg.A, m_reg.L); }
static void cpu_opcode_OR_rA_iHL(void) {
    cpu_generic_OR_r8_u8(&m_reg.A, mmu_read(BUS_CPU, m_reg.HL));
}
static void cpu_opcode_OR_rA_rA(void) { cpu_generic_OR_r8_u8(&m_reg.A, m_reg.A); }
