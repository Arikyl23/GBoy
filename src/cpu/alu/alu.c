...
struct alu_result alu_adc_8b(const byte lhs, const byte rhs, const bool carry) {
  ...
  rtn.flags.n = false;
  rtn.flags.h =
      ((lhs & 0x0F) + (rhs & 0x0F) + ((carry == true) ? 1 : 0)) > 0x0F;
...
struct alu_result alu_add_16b(const word lhs, const word rhs) {
  ...
  return (struct alu_result){.result.word = (rtn_high.result.byte << 8) |
                                            (rtn_low.result.byte),
                             .flags = rtn_high.flags};
}
...
struct alu_result alu_da(const byte bcd_value, const bool n_flag,
                         const bool h_flag, const bool c_flag) {
  struct alu_result rtn = {0};
  rtn.flags.c = c_flag;
  byte adjustment = 0;
  if (n_flag == true) {
    ...
    rtn.result.byte = bcd_value + adjustment;
    rtn.flags.c =
        ((((word)bcd_value + (word)adjustment) > 0x00FF) ? true : c_flag);
  }
  rtn.flags.z = (rtn.result.byte == 0);
  rtn.flags.h = false;
  return rtn;
